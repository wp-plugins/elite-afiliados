...
function elite($colunas,$catid,$class="elite",$width=30,$height=30,$cellspacing=4) {
	...
	foreach($bk as $link) {
		echo "<td>";
		echo "<a href=\"{$link['link_url']}\" target=\"_blank\" title=\"{$link['link_description']}\"><img src=\"{$link['link_image']}\" width=\"$width\" height=\"$height\"></a>";
		echo "</td>\n";
		...
		$faltam = $colunas-$nm; // ve qts faltam
			if($faltam>0) { // se ainda faltar algum
				while($faltam>=1) { // mostra vazio ate completar 9
					echo "<td><img src=\"".get_bloginfo('template_directory')."/images/spacer.gif\" width=\"$width\" height=\"$height\" class=\"blank\" /></a></td>\n";
					$faltam--;
				}
			}
		}
...
function elite_widget($args) {
	...
	echo "{$before_widget}{$before_title}{$options['title']}{$after_title}";
	  elite($options['colunas'],$options['linkscat'],$options['class'],$options['width'],$options['height'],$options['spacing']);
	echo $after_widget;
}
